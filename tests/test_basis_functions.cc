...
class BFGradientTest : public TestWithParam<std::string> {
  protected:
    virtual void SetUp() {
      ...
      M = 20;
      e = 1e-8;
      wrappedCovarianceFunction = covFactory.create(n, "CovSum ( CovSEiso, CovNoise)");
      covf = factory.createBasisFunction(GetParam(), M, wrappedCovarianceFunction);
...
TEST_P(BFGradientTest, EqualToNumerical) {
  ...
  for (int i=0; i<param_dim; ++i) {
    if (grad(i) == 0.0) ASSERT_NEAR(numerical_gradient(i), 0.0, 1e-2) << "Parameter number: " << i;
    else ASSERT_NEAR((numerical_gradient(i)-grad(i))/grad(i), 0.0, 1e-2) << "Parameter number: " << i;
  }
}

//TEST_P(GradientTest, InverseWeightPriorEqualToNumerical) {
...
//}
//
//TEST_P(GradientTest, BasisFunctionEqualToNumerical) {
//	size_t M = covf->getNumberOfBasisFunctions();
...
//	for(size_t j=0; j < M; j++){
//			if (grad(j) == 0.0) ASSERT_NEAR(numeric_gradient(j), 0.0, 1e-2);
//			else ASSERT_NEAR((numeric_gradient(j)-grad(j))/grad(j), 0.0, 1e-2);
//	}
//  }
//}

INSTANTIATE_TEST_CASE_P(BasisFunction, BFGradientTest, Values(
          "SparseMultiScaleGP",
		  "SparseMultiScaleGP"
          ));
