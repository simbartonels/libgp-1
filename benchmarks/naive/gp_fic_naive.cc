...
FICnaiveGaussianProcess::FICnaiveGaussianProcess(size_t input_dim,
...
FICnaiveGaussianProcess::~FICnaiveGaussianProcess() {
}
double FICnaiveGaussianProcess::var_impl(const Eigen::VectorXd &x_star) {
...
void FICnaiveGaussianProcess::computeCholesky() {
...
void FICnaiveGaussianProcess::updateCholesky(const double x[], double y) {
//Do nothing and just recompute everything.
//TODO: might be a slow down in applications!
	cf->loghyper_changed = true;
}
void FICnaiveGaussianProcess::update_k_star(const Eigen::VectorXd &x_star) {
	k_star = bf->computeBasisFunctionVector(x_star);
}
void FICnaiveGaussianProcess::update_alpha() {
	size_t n = sampleset->size();
	if (n > r.size()) {
		r.resize(n);
	}
...
double FICnaiveGaussianProcess::log_likelihood_impl() {
	double t = 0;
	double t3 = 0;
...
Eigen::VectorXd FICnaiveGaussianProcess::log_likelihood_gradient_impl() {
...
// W = chol(Kuu+W*W'+snu2*eye(nu))'\Ku; % inv(K) = inv(G) - inv(G)*W'*W*inv(G);
	W = (W * W.transpose() + bf->getInverseOfSigma()).selfadjointView<
			Eigen::Lower>().llt().matrixL().solve(Phi);
	const std::vector<double>& targets = sampleset->y();
	size_t n = sampleset->size();
	Eigen::Map<const Eigen::VectorXd> y(&targets[0], n);
	Eigen::VectorXd al(n);
...
//TODO: check if these allocations are necessary and if move them to the constructor
		...
		Eigen::VectorXd v = ddiagK.col(i).transpose()
				- R.cwiseProduct(B).colwise().sum();
