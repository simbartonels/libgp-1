...
AbstractGaussianProcess::AbstractGaussianProcess(size_t input_dim,
		std::string covf_def) {
...
void AbstractGaussianProcess::write(const char * filename) {
	...
	outfile << std::endl << std::endl << "# data (target value in first column)"
			<< std::endl;
...
double AbstractGaussianProcess::f(const Eigen::VectorXd & x) {
	if (sampleset->empty())
		return 0;
	compute();
	update_k_star_preprocessing(x, false);
...
void AbstractGaussianProcess::grad_f(const Eigen::VectorXd & x, Eigen::VectorXd &grad) {
	...
	compute();
	update_k_star_preprocessing(x, true);
	cf->compute_dkdx(x, k_star, sampleset, JT);
	grad = JT * alpha;
}
...
void AbstractGaussianProcess::grad_var(const Eigen::VectorXd & x,
		Eigen::VectorXd &grad) {
	if (sampleset->empty()) {
		grad = Eigen::VectorXd::Zero(input_dim);
		return;
